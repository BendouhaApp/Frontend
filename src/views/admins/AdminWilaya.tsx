...
function CommuneForm({
  ...
  const [freeShipping, setFreeShipping] = useState(commune?.free_shipping ?? false);
  const [homeEnabled, setHomeEnabled] = useState(commune?.home_delivery_enabled ?? true);
  const [homePrice, setHomePrice] = useState(commune?.home_delivery_price?.toString() ?? "0");
  const [officeEnabled, setOfficeEnabled] = useState(commune?.office_delivery_enabled ?? true);
  const [officePrice, setOfficePrice] = useState(commune?.office_delivery_price?.toString() ?? "0");
  ...
  return (
    <form onSubmit={submit} className="space-y-3 rounded-lg border bg-neutral-50 p-3">
      <div className="grid grid-cols-2 gap-2">
        <input value={name} onChange={(e) => setName(e.target.value)} placeholder="Nom technique (bir-khadem)" className="w-full rounded-lg border px-3 py-2 text-sm" required />
        <input value={displayName} onChange={(e) => setDisplayName(e.target.value)} placeholder="Nom affiche (Bir Khadem)" className="w-full rounded-lg border px-3 py-2 text-sm" required />
      </div>
      <div className="grid grid-cols-2 gap-2">
        <input type="number" min="0" step="0.01" value={homePrice} onChange={(e) => setHomePrice(e.target.value)} disabled={!homeEnabled} placeholder="Prix domicile" className="w-full rounded-lg border px-3 py-2 text-sm" />
        <input type="number" min="0" step="0.01" value={officePrice} onChange={(e) => setOfficePrice(e.target.value)} disabled={!officeEnabled} placeholder="Prix bureau" className="w-full rounded-lg border px-3 py-2 text-sm" />
      ...
      <div className="grid grid-cols-4 gap-2 text-sm">
        <label className="flex items-center justify-between rounded-lg border px-2 py-1.5">Actif <input type="checkbox" checked={active} onChange={(e) => setActive(e.target.checked)} /></label>
        <label className="flex items-center justify-between rounded-lg border px-2 py-1.5">Gratuite <input type="checkbox" checked={freeShipping} onChange={(e) => setFreeShipping(e.target.checked)} /></label>
        <label className="flex items-center justify-between rounded-lg border px-2 py-1.5">Domicile <input type="checkbox" checked={homeEnabled} onChange={(e) => setHomeEnabled(e.target.checked)} /></label>
        <label className="flex items-center justify-between rounded-lg border px-2 py-1.5">Bureau <input type="checkbox" checked={officeEnabled} onChange={(e) => setOfficeEnabled(e.target.checked)} /></label>
      </div>
      <div className="flex gap-2">
        <Button type="button" variant="outline" className="flex-1" onClick={onCancel}>Annuler</Button>
        <Button type="submit" className="flex-1" disabled={loading}>{loading && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}{commune ? "Mettre a jour" : "Ajouter"}</Button>
      </div>
    </form>
  );
}
...
export default function AdminWilaya() {
  ...
  const [editingCommune, setEditingCommune] = useState<ShippingCommune | null>(null);
  ...
  const filtered = useMemo(() => {
    ...
    return list.filter((z) => z.display_name.toLowerCase().includes(q) || z.name.toLowerCase().includes(q));
  }, [zones, filter, search]);
  ...
  const saveZone = async (payload: Record<string, unknown>) => {
    ...
    setError("");
    try {
      if (editingZone) await api.patch(`admin/shipping-zones/${editingZone.id}`, payload);
  ...
  const saveCommune = async (payload: Record<string, unknown>) => {
    ...
    setCommunesError("");
    try {
      if (editingCommune) await api.patch(`admin/shipping-zones/${selectedZone.id}/communes/${editingCommune.id}`, payload);
      else await api.post(`admin/shipping-zones/${selectedZone.id}/communes`, payload);
    ...
    <div className="min-h-screen bg-neutral-50">
      <div className="mx-auto max-w-7xl space-y-4 px-4 py-6 md:px-6">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <h1 className="text-2xl font-semibold text-neutral-900">Zones de livraison</h1>
            <p className="text-sm text-neutral-600">Gestion des wilayas et communes</p>
          </div>
          <div className="flex gap-2">
            <Button variant="outline" onClick={loadZones} disabled={loading}><RefreshCcw className={cn("mr-2 h-4 w-4", loading && "animate-spin")} />Actualiser</Button>
            <Button onClick={() => { setEditingZone(null); setShowZoneModal(true); }}><Plus className="mr-2 h-4 w-4" />Ajouter</Button>
          </div>
        </div>
        ...
        <div className="flex flex-wrap gap-2">
          <div className="relative min-w-[260px] flex-1"><Search className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-neutral-400" /><input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Rechercher une wilaya..." className="w-full rounded-lg border px-9 py-2 text-sm" /></div>
          <div className="flex rounded-lg border bg-white p-1 text-sm">
            <button className={cn("rounded px-3 py-1.5", filter === "all" && "bg-blue-600 text-white")} onClick={() => setFilter("all")}>Toutes</button>
            <button className={cn("rounded px-3 py-1.5", filter === "active" && "bg-blue-600 text-white")} onClick={() => setFilter("active")}>Actives</button>
            <button className={cn("rounded px-3 py-1.5", filter === "inactive" && "bg-blue-600 text-white")} onClick={() => setFilter("inactive")}>Inactives</button>
          </div>
        </div>

        {loading ? <div className="space-y-3 rounded-lg bg-white p-4">{[1,2,3].map((i) => <Skeleton key={i} className="h-12 w-full rounded-lg" />)}</div> : (
